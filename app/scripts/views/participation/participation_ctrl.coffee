...
.controller 'CreateParticipationDirectiveController', ($route, $scope,
  ...
  $scope.saveParticipation = ->
    ...
    if (not $scope.participationForm.$valid or not $scope.participationForm.$dirty)
      return
...
.controller 'EditParticipationDirectiveController', ($route, $scope,
  ...
  $scope.saveParticipation = ->
    ...
    for key, value of $scope.participationForm
      if key.charAt(0) != '$' and value.$dirty
        if key == 'temperature_debut' or key == 'temperature_fin' or key == 'vent' or key == 'couverture'
          if $scope.participation.meteo[key]?
            payload.meteo[key] = $scope.participation.meteo[key]
        else if key == 'detecteur_enregistreur_numero_serie' or key == 'micro0_position' or
            key == 'micro0_numero_serie' or key == 'micro0_hauteur' or key == 'micro1_position' or
